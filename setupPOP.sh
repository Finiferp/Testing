...
println_red "In the installation process you will need the following values:"
println_red "Wifi Password"
println_red "Wifi Interface"
println_red "MQTT Username"
println_red "MQTT Password"
println_red "MQTT Node-Red Username"
println_red "MQTT Node-Red Password"
println_red "SSH Passphrase"

println_red "!!! Please confirm you have the above values before proceeding !!!"
...
# Set fix ip address
# Prompt for network interface and static IP configuration details
...
read -p "Enter the DNS servers (e.g., 8.8.8.8,8.8.4.4): " DNS

# Check if the interface exists
if ! nmcli device status | grep -q "$INTERFACE"; then
    echo "Error: Interface $INTERFACE not found. Please check the name and try again."
    exit 1
fi
...
echo "Configuration complete. Validating the settings..."
